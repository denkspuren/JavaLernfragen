...
ifdef::solution[]
.Antwort
Bei jedem Aufruf einer Methode merkt sich die Java-Ausführungsmaschinerie (die Java Virtual Machine, JVM), an welcher Stelle das Programm nach dem Ende der Methode fortgesetzt werden muss. Außerdem merkt sich die JVM den Zustand lokaler Variablen. Sowohl die Rücksprungadresse als auch die lokalen Variablen werden im sogenannten "Methodenaufrufstapel" (_callstack_) gespeichert. Der dafür vorgesehene Speicherplatz ist endlich. Wächst der Aufrufstapel über das Speicherlimit hinaus, wird die Programmausführung abgebrochen.
endif::solution[]
